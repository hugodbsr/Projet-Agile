...
public class Historique {
    ...
    public static boolean createGameFile(String name){
        Historique.playername = name;
        Historique.filePath = System.getProperty("user.dir") + "/res/historique/" + LocalDateTime.now().toString().substring(0,19).replace("T", "_") + "_joueur_" + playername + ".txt";
        Historique.file = new File(filePath);
        try(BufferedWriter bw = new BufferedWriter(new FileWriter(Historique.file))){
            bw.write("-------------------------------------------------------");
            bw.newLine();
            bw.write("Partie de " + playername + " le " + LocalDateTime.now().toString());
            bw.newLine();
            bw.write("-------------------------------------------------------");
            bw.newLine();
            bw.newLine();
    ...
    public static boolean addPlateau(Plateau playerPlateau, Plateau botPlateau){
        String msg = "Bateaux de " + Historique.playername + '\n'
            + playerPlateau.getStringPlateau(botPlateau).replace("\u001B[", "").replace("0m", "").replace("31m", "").replace("32m", "").replace("33m", "").replace("34m", "") + "\n\n"
            + "Bateaux de l'ORDI" + '\n'
            + botPlateau.getStringPlateau(playerPlateau).replace("\u001B[", "").replace("0m", "").replace("31m", "").replace("32m", "").replace("33m", "").replace("34m", "") + "\n\n\n";
